...
static void wpmfs_inode_table_crawl_recursive(struct super_block *sb,
  ...
  if (height == 0) {
    ...
    for (i = 0; i < inodes_per_block; i++) {
      pi = (struct pmfs_inode *)((void *)node + PMFS_INODE_SIZE * i);
      if (le16_to_cpu(pi->i_links_count) == 0 &&
          (le16_to_cpu(pi->i_mode) == 0 || le32_to_cpu(pi->i_dtime))) {
        /* Empty inode */
        continue;
      }
...
int wpmfs_setup_blocknode_map(struct super_block *sb) {
  ...
  if (!bm.bitmap_4k) goto skip;

  wpmfs_inode_table_crawl_recursive(sb, &bm, le64_to_cpu(pi->root), pi->height,
                                    pi->i_blk_type);

  /* Reserving tow inodes - Inode 0 and Inode for datablock */
  sbi->s_free_inodes_count =
      sbi->s_inodes_count - (sbi->s_inodes_used_count + 2);
...
static bool pmfs_can_skip_full_scan(struct super_block *sb) {
  ...
  last_blocknr = (le64_to_cpu(pi->i_size) - 1) >> pmfs_inode_blk_shift(pi);
...
void pmfs_save_blocknode_mappings(struct super_block *sb) {
  ...
  num_blocks =
      ((sbi->num_blocknode_allocated * sizeof(struct pmfs_blocknode_lowhigh) -
        1) >>
       sb->s_blocksize_bits) +
      1;
  ...
  pmfs_add_logentry(sb, trans, &super->s_wtime, PMFS_FAST_MOUNT_FIELD_SIZE,
                    LE_DATA);
...
static void pmfs_inode_crawl_recursive(struct super_block *sb,
  ...
  for (i = 0; i < (1 << META_BLK_SHIFT); i++) {
    if (node[i] == 0) continue;
    pmfs_inode_crawl_recursive(sb, bm, le64_to_cpu(node[i]), height - 1, btype);
  }
}
...
static void pmfs_inode_table_crawl_recursive(struct super_block *sb,
  ...
  for (i = 0; i < (1 << META_BLK_SHIFT); i++) {
    if (node[i] == 0) continue;
    pmfs_inode_table_crawl_recursive(sb, bm, le64_to_cpu(node[i]), height - 1,
                                     btype);
  }
}
...
static int pmfs_alloc_insert_blocknode_map(struct super_block *sb,
  ...
  if (free_blocknode) pmfs_free_blocknode(sb, free_blocknode);
...
int pmfs_setup_blocknode_map(struct super_block *sb) {
  ...
  if (!bm.bitmap_4k || !bm.bitmap_2M || !bm.bitmap_1G) goto skip;

  /* Clearing the datablock inode */
  pmfs_clear_datablock_inode(sb);

  pmfs_inode_table_crawl_recursive(sb, &bm, le64_to_cpu(pi->root), pi->height,
                                   pi->i_blk_type);
